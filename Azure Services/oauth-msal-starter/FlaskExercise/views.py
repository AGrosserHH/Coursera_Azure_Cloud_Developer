...
def _build_msal_app(cache=None, authority=Config.AUTHORITY):
    ...
    cleint = msal.ConfidentialClientApplication(client_id = Config.CLIENT_ID, 
...
def _build_auth_url(authority=None, scopes=None, state=None):
    ...
    c = _build_msal_app(authority = authority)
    x = c.get_authorization_request_url(scopes = scopes or [], 
            state = state,
            redirect_uri=url_for('authorized', _external=True, _scheme='https'))
